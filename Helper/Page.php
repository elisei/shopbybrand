...
class Page extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function prepareResultPage(Action $action, $pageId = null)
    {
        ...
        $this->_coreRegistry->register('current_brand', $this->_page);
        ...
        $resultPage->addHandle('brand_page_view');
        $resultPage->addPageLayoutHandles(['id' => str_replace('/', '_', $this->_page->getIdentifier())]);

        $this->_eventManager->dispatch(
            'brand_page_render',
            ['brand' => $this->_page, 'controller_action' => $action, 'request' => $this->_getRequest()]
        );
        ...
        if ($contentHeadingBlock) {
            $contentHeading = $this->_escaper->escapeHtml($this->_page->getTitle());
