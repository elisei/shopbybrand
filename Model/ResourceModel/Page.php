...
namespace O2TI\ShopByBrand\Model\ResourceModel;

use O2TI\ShopByBrand\Api\Data\PageInterface;
use O2TI\ShopByBrand\Model\Page as BrandPage;
...
class Page extends AbstractDb
{
     ...
     * Load an object
     *
     * @param BrandPage|AbstractModel $object
     * @param mixed $value
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['brand_page_store' => $this->getTable('brand_page_store')],
                $this->getMainTable() . '.' . $linkField . ' = brand_page_store.' . $linkField,
                []
            )
                ->where('is_active = ?', 1)
                ->where('brand_page_store.store_id IN (?)', $storeIds)
                ->order('brand_page_store.store_id DESC')
                ->limit(1);
        }
    ...
    protected function _getLoadByIdentifierSelect($identifier, $store, $isActive = null)
    {
        ...
        $select = $this->getConnection()->select()
            ...
            ->join(
                ['cps' => $this->getTable('brand_page_store')],
    ...
    /**
     * Retrieves brand page title from DB by passed identifier.
     *
     * @param string $identifier
     * @return string|false
     */
    public function getBrandPageTitleByIdentifier($identifier)
    {
    ...
    /**
     * Retrieves brand page title from DB by passed id.
     *
     * @param string $id
     * @return string|false
     */
    public function getBrandPageTitleById($id)
    {
    ...
    /**
     * Retrieves brand page identifier from DB by passed id.
     *
     * @param string $id
     * @return string|false
     */
    public function getBrandPageIdentifierById($id)
    {
        $connection = $this->getConnection();
        $entityMetadata = $this->metadataPool->getMetadata(PageInterface::class);
    ...
    public function lookupStoreIds($pageId)
    {
        ...
        $select = $connection->select()
            ->from(['cps' => $this->getTable('brand_page_store')], 'store_id')
            ->join(
